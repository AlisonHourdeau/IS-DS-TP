...
#Q2 d)
library(stringr) #manipulation des chaines de caracteres
...
Quadratique <- function(x,w){
  produit<- w*(x**2)  # c'est un exemple de vectorisation
  somme<-sum(produit)
  racine<- somme*(0.5)
  return( racine)
}
...
# Q6
E<-seq(0,50,2) # equivalent à E<-seq(from=0,to=50,by=2)
...
# X un vecteur et x1,...,xn ses élements, alors f(X) renvoie c(f(x1),...,f(xn))
...
#Q6
P1 <- P[, which(apply(P<5, 2, all)), drop=FALSE]

#Q7
P2 <- P[which(apply(P>0, 1, all)), , drop=FALSE]
...
#Q6
informations <- informations[order(informations$taille),]

#Q7
j<-which(informations$sexe %in% "H") # lignes ou l'individu est de sexe masculin

# Par defaut on applique la formule dediee aux femmes
informations$poids<-informations$taille*100*0.5-20

# Ensuite on applique la formule aux hommes
informations$poids[j]<-informations$taille[j]*100*0.5-10
